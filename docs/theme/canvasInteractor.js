...
globalThis.canvasInteractor = globalThis.canvasInteractor || {
    ...
    prototype: {
        constructor(ctx, {x,y,scl,cartesian}) {
            ...
            globalThis.addEventListener("keydown", this, false);
            globalThis.addEventListener("keyup", this, false);
        ...
        deinit() {
            ...
            globalThis.removeEventListener("keydown", this, false);
            globalThis.removeEventListener("keyup", this, false);
        ...
        handleEvent(e) {
            const whitelist = ['wheel', 'keydown', 'keyup'];
            if (e.type in this && (e.isPrimary || whitelist.includes(e.type))) {  // can I handle events of type e.type .. ?
        ...
        keydown() {
            const k = this.keys;
            const b = this.evt.btn;
            if (!k.includes(b)) k.push(b);
        },
        keyup() {
            if (!this.keys) return;
            this.keys = this.keys.filter(e => e !== this.evt.btn);
        },
        wheel() {
